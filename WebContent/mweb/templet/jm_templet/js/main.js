$(function(){
	...
	$("#jmAudio").on("canplay",function(){
		$(".fullTime").text(formatTime($("#jmAudio")[0].duration));
	});
	
	//实时获取播放时长和改变进度条进度
	$("#jmAudio")[0].addEventListener("timeupdate",function(){
	    if (!isNaN($(this)[0].duration)) {
	        ...
	        //播放时长
	    	$(".playTime").text(formatTime(Math.round($("#jmAudio")[0].currentTime)));
	    };
	},false);
